...
- ✅ **v0.1 fertig und live**, statische Seite (kein Server, kein Build-Schritt zur Laufzeit) — GitHub Pages laut README auf `Settings → Pages → Deploy from a branch → main /docs` konfiguriert; die konkrete Pages-URL hier nicht erneut gegengeprüft (GitHub-API-Rate-Limit beim Schreiben dieses Standes), aber im echten Browser von Flo wiederholt getestet und bestätigt (Screenshots über mehrere Feedback-Runden).
